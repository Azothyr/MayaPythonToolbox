...
class WeightPaintHelper:
    ...
    def get_ctrl_assist_attrs(self):
...
if __name__ == "__main__":
    ...
    # obj = "_".join([i.capitalize() for i in "l_leg_03".split("_")])
    # rem_obj = ckecker_obj(ckecker_obj("_".join([i.capitalize() for i in "l_leg_03".split("_")]), obj),
    #                       None)
    ...
    # Store the current selection, which might contain objects or components other than vertices
    selection = cmds.ls(selection=True)

    # Clear the current selection
    cmds.select(clear=True)

    # Select all vertices of the mesh "Proxy_Skin_Geo" that are not in the current selection
    cmds.select(cmds.lsThroughFilter("Proxy_Skin_Geo.vtx[*]"))
