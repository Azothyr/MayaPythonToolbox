...
class LimbTwistManager:
    ...
    def create_upper_twist_system(self):
        ...
        cmds.select(clear=True)
        self.create_falloff_twist(self.u_name, self.top_jnt_xform, self.u_aim, self.u_target, self.u_loc_grp)
        cmds.select(clear=True)
        self.create_twist_joints(self.u_name, self.u_loc_grp, self.u_target, self.rk_top_jnt)
        cmds.select(clear=True)
    ...
    def create_upper_twist_base(self):
        ...
        self.u_up_xform.add_in_local('translate', z=-5)
        ...
        # Parent constraint keeping the up locator's position relative to the cmds.parentConstraint(
        # self.rk_pivot_jnt, self.u_up, mo=True, name=f"parent_constraint__from_{self.rk_pivot_jnt}")
    ...
    def create_upper_limb_helpers(self):
        ...
        self.help_jnt1_xform.add_in_world('translate', z=-3)
        ...
        self.loc_pv_xform.add_in_world('translate', y=3)
    ...
    def create_lower_twist_base(self):
        ...
        self.l_up_xform.add_in_local('translate', y=5)
    ...
    def clean_up(self):
        _group = cmds.group(name="Limb_Twist_Grp", empty=True) if not \
            cmds.objExists("Limb_Twist_Grp") else "Limb_Twist_Grp"
...
if __name__ == "__main__":
    ...
    creator = LimbTwistManager("L_Arm", "L_Upper_Arm", "L_ForeArm", selection[0],
                               selection[1], selection[2], selection[3], twist_count=1,
